...
def ALU1bit(a, b, carryin, binvert, operation, result, carryout):
    ...
    def comb_mux_2_1():
        ...
        if binvert == 0:
            mux1_out.next = b #if the invert is 0 it just wants b
        else:
            mux1_out.next = notb #if the invert is 1 it wants not b
    ...
    @always_comb
    def comb_and():
        and_out.next = a and mux1_out #simple and
    ...
    @always_comb
    def comb_or():
        or_out.next = a or mux1_out #simple or
    ...
    @always_comb
    def comb_adder():
        adder_sum.next = a ^ mux1_out ^ carryin #from the slides
        carryout.next = (a & mux1_out) | (carryin & (a ^ mux1_out)) #clearing the other bits and setting the carryout

    # 4-1 mux to generate result
    @always_comb
    def comb_mux_4_1():
        if operation == 0:
            result.next = and_out #easy conditional, follow the provided image.
        elif operation == 1:
            result.next = or_out
        elif operation == 2:
